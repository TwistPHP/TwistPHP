...
<div class="grid-33 tablet-grid-33 mobile-grid-100">
	...
	<p class="{data:database-debug=='Off'?'success':'warning'}"><small class="float-right">[<a href="{route:registered_uri}/update-setting?setting=DATABASE_DEBUG&setting_value={data:database-debug=='Off'?'1':'0'}">Turn {data:database-debug=='Off'?'On':'Off'}</a>]</small><strong>Database Debug:</strong> {data:database-debug}</p>
	<p class="{data:release-channel=='Stable'?'success':'warning'}"><small class="float-right">[<a href="{route:registered_uri}/update-setting?setting=RELEASE_CHANNEL&setting_value={data:release-channel=='Stable'?'Nightly':'Stable'}">Use {data:release-channel=='Stable'?'Nightly':'Stable'}</a>]</small><strong>Release Channel:</strong> {data:release-channel}</p>
</div>
<div class="clear"></div>
